...
def execute(filters=None):
    ...
    for emp in employees:        
        for month_idx in range(1, no_of_months):                
            month_date_string = cstr(p10a_tdc_year) + "-" + cstr(month_idx) + "-" + cstr(1)
            ...
            if (basic_salary_amt > 0):
                e3_defined_contribution_retirement_scheme_amt = get_p10a_tax_deduction_card_fixed_component_amt(
                                                              p10a_tax_deduction_card_type[6])
            else:
...
def validate_dates(p10a_year_start_date, p10a_year_end_date):
    if not p10a_year_start_date or not p10a_year_end_date:
        frappe.throw("P10A Year Start Date and P10A Year End Date are mandatory")
    if p10a_year_end_date < p10a_year_start_date:
        frappe.throw("P10A Year End Date cannot be less than P10A Year Start Date")
...
def get_p10a_tax_deduction_card_amt(filters, employee, month_start_date, month_end_date, p9a_tax_deduction_card_type, currency, company_currency): 
    currency_filter = ''
    ...
    ss_p10a_tax_deduction_card_component = frappe.db.sql("""
        SELECT
    ...
    p10a_tax_deduction_card_amount = 0
    for d in ss_p10a_tax_deduction_card_component:
        if currency != company_currency:
            p10a_tax_deduction_card_amount += flt(d.amt) * flt(d.exchange_rate if d.exchange_rate else 1)
        else:
...
def get_p10a_tax_deduction_card_gross_pay(filters, employee, month_start_date, month_end_date, currency, company_currency): 
    ...
    ss_p10a_tax_deduction_card_gross_pay = frappe.db.sql("""
        SELECT
            ss.employee, ss.docstatus,
	        ss.currency, ss.start_date,
            ss.end_date, ss.company,
    ...
    p10a_tax_deduction_card_gross_pay = 0
    for d in ss_p9a_tax_deduction_card_gross_pay:
        if currency != company_currency:
            p10a_tax_deduction_card_gross_pay += flt(d.amt) * flt(d.exchange_rate if d.exchange_rate else 1)
        else:
            p10a_tax_deduction_card_gross_pay += flt(d.amt)

    return p10a_tax_deduction_card_gross_pay

def get_p10a_tax_deduction_card_fixed_component_amt(p10a_tax_deduction_card_type):
    p10a_tax_deduction_card_fixed_component = frappe.db.sql("""
            SELECT
                IFNULL(sc.amount,0) as amt
		    FROM
                `tabSalary Component` sc
            WHERE
                sc.p10a_tax_deduction_card_type = %(p10a_tax_deduction_card_type)s
            """, {
                'p10a_tax_deduction_card_type': p10a_tax_deduction_card_type
            }, as_dict=True)

    p10a_tax_deduction_card_fixed_component_amt = 0
    for d in p10a_tax_deduction_card_fixed_component:
        p10a_tax_deduction_card_fixed_component_amt += flt(d.amt)

    return p10a_tax_deduction_card_fixed_component_amt